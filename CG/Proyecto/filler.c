...
EdgeTableTuple EdgeTable[H_SIZE], ActiveEdgeTuple;
...
void ScanlineFill(){
    ...
    for (i=0; i<H_SIZE; i++)//4. Increment y by 1 (next scan line)
    {
...
void drawPolyDino(Polygon p){

    int count = 0,x1,y1,x2,y2;

    for (count = 1; count < p.length; count++) {
      if (count>2){
          x1 = x2;
          y1 = y2;
      }

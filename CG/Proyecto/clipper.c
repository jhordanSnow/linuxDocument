...
Polygon clipPolygon(Polygon p, Window window){
  //vec_t polygon = castPolygon(p);
  int delta = 0;
  int i;
	int margin = 5;
  vec_t c[4] = {
    (vec_t){margin, margin},
    (vec_t){window.resolution.x-margin, margin},
    (vec_t){window.resolution.x-margin, window.resolution.y-margin},
    (vec_t){margin, window.resolution.y - margin},
  };

  vec_t s[p.length + 1];
  for (int i = 0; i < p.length; i++) {
    s[i] = (vec_t){p.points[i].x, p.points[i].y};
  }

  #define clen (sizeof(c)/sizeof(vec_t))
  #define slen (sizeof(s)/sizeof(vec_t))
	poly_t clipper = {4, 0, c};
	poly_t subject = {p.length, 0, s};

	poly res = poly_clip(&subject, &clipper);

  vector* vectorList = (vector*)malloc(sizeof(vector) * res->len);
	for (i = 0; i < res->len; i++){
    vectorList[i] = (vector) { (int)res->v[i].x, (int)res->v[i].y };
  }
